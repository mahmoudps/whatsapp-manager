...
export async function initializeWebSocketServer(port: number = Number(PORT)): Promise<WebSocketServerInstance> {
  ...
  try {
    ...
    io.on("connection", (socket: any) => {
      ...
      // مغادرة غرفة الجهاز
      socket.on("leave_device", (deviceId: number) => {
        if (deviceId) {
    ...
    setInterval(() => {
      ...
      connectedClients.forEach((client: any, clientId: string) => {
...
export async function getWebSocketServer(): Promise<WebSocketServerInstance> {
  if (!wsServerInstance || !wsServerInstance.isRunning) {
