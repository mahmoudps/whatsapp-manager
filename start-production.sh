...
# تحميل المتغيرات من الملف .env إن وجد دون الكتابة فوق القيم المحددة مسبقاً
if [ -f .env ]; then
  while IFS='=' read -r key value; do
    # تخطي الأسطر الفارغة أو المعلقة بالتعليقات
    if [ -n "$key" ] && [[ $key != \#* ]]; then
      # استخدم القيمة من الملف فقط إذا لم تكن متغيرة في البيئة مسبقاً
      if [ -z "${!key}" ]; then
        export "$key"="$value"
      fi
    fi
