...
## التحديث والصيانة
بعد جلب التحديثات من المستودع يفضل تحديث ملف البيئة لإضافة أي متغيرات جديدة. يمكن تنفيذ:
```bash
wa-manager rebuild
# أو
npm run setup
```
لضمان إعادة توليد ملف `.env` ووجود المتغيرات الأساسية مثل `JWT_SECRET`.
