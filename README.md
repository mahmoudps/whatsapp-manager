...
## تخصيص الألوان
القيم الأساسية للوضعين الفاتح والداكن محفوظة في الملف `lib/theme.ts`،
وهي تُعرِّف الرموز التي يستخدمها Tailwind.
يُستورد الملف `styles/themes/colors.css` داخل `styles/globals.css` لتوفير
المتغيرات، ولا تتم إعادة توليده تلقائيًا؛ لذا يمكن تعديله يدويًا عند
الحاجة.
